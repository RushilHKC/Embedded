//Creator: Arnav Khanade
//Date: 16/10/2024

//TX OK/ Rx OK


//Global:gInterruptLine1Status, gServiceInt, RxData, bufferIndex
...
DL_MCAN_RxBufElement grxMsg;
DL_MCAN_RxFIFOStatus grxFS;
uint8_t gRxData[256]={0};
volatile uint32_t gbufferIndex=0;
...
//Function to initialize CAN Msg Frame
void CAN_initMsgFrame(DL_MCAN_TxBufElement* txMsg,uint32_t id,uint32_t size,uint32_t* data){
...
//Function to  transmit data in CAN
void CAN_transmitMsg(DL_MCAN_TxBufElement* txMsg){
...
//function to receive data on CAN
void CAN_receiveMsg(){
    gServiceInt  = false;
    grxFS.fillLvl = 0;

    if ((gInterruptLine1Status & MCAN_IR_RF0N_MASK) == MCAN_IR_RF0N_MASK) {
        grxFS.num = DL_MCAN_RX_FIFO_NUM_0;
        while ((grxFS.fillLvl) == 0) {
            DL_MCAN_getRxFIFOStatus(MCAN0_INST, &grxFS);
        }

        DL_MCAN_readMsgRam(
            MCAN0_INST, DL_MCAN_MEM_TYPE_FIFO, 0U, grxFS.num, &grxMsg);

        DL_MCAN_writeRxFIFOAck(MCAN0_INST, grxFS.num, grxFS.getIdx);
        ...
        if (ID_MODE_EXTENDED == idMode) {
            ...
            id = ((grxMsg.id & (uint32_t) 0x1FFC0000) >> (uint32_t) 18);

        }
        for(int i=0;i<grxMsg.dlc;i++){
            gRxData[gbufferIndex++]=grxMsg.data[i];
